...
export class MongoDataAccess {
    static url: string = 'mongodb://127.0.0.1:27017/monitor';
    dbConnection: any = null;

    // Open the MongoDB connection.
    public openDbConnection() {
        if (this.dbConnection == null) {
            MongoClient.connect(MongoDataAccess.url, (err, db) => {
    ...
    //
    public getCount(collection: string): any {
        return this.getDocumentCount(collection);
    }
    
    public getClientsList(): any {
    ...
    public getClients(): any {
        var deferred = Q.defer();
        if (this.dbConnection) {
            var cursor = this.dbConnection.collection('summary_inbox').disctinct('clienteId');
            cursor.each((err:any, document:any) => {
                assert.equal(err, null);
                if (err) {
                    deferred.reject(new Error(JSON.stringify(err)));
                }
